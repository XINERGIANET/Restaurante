<div class="space-y-8">
    ...
    <div>
        ...
        <div class="bg-gray-50 rounded-xl p-5 border border-gray-100 dark:bg-white/[0.02] dark:border-gray-800 grid grid-cols-1 gap-5">
            ...
            <div class="col-span-full">
                ...
                <input type="text" name="comment" required placeholder="Ej: Pago de servicios de luz..." 
                    ...
                    :class="formConcept === 'Apertura de caja' || formConcept === 'Cierre de caja' ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-white dark:bg-dark-900'"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select name="shift_id" required class="h-11 w-full rounded-lg border-gray-200 bg-white pl-10 pr-4 text-sm text-gray-800 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:bg-dark-900 dark:text-white/90 appearance-none transition-all">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select name="payment_concept_id" required x-model="formConceptId" class="h-11 w-full rounded-lg border-gray-200 bg-white pl-10 pr-4 text-sm text-gray-800 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:bg-dark-900 dark:text-white/90 appearance-none transition-all">
    ...
    <div x-data="{ 
        ...
        <div class="flex flex-col sm:flex-row sm:items-end justify-between mb-6 pb-4 border-b border-gray-100 dark:border-gray-800">
            <div>
            ...
            <div class="mt-4 sm:mt-0 text-right bg-brand-50/50 dark:bg-brand-900/10 px-4 py-2 rounded-lg">
                <span class="block text-xs text-brand-600 dark:text-brand-400 font-medium uppercase tracking-wider">Total a Pagar</span>
        ...
        <div class="space-y-4">
            <template x-for="(row, index) in rows" :key="row.id">
                ...
                <div class="relative bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 transition-all hover:border-brand-300 dark:hover:border-brand-700">
                    
                    {{-- Botón Eliminar (Visible si hay > 1) --}}
                    <template x-if="rows.length > 1">
                        <button type="button" @click="removeRow(index)" class="absolute top-4 right-4 text-gray-400 hover:text-red-500 transition-colors p-1">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
                        ...
                        <div class="md:col-span-5 flex gap-3">
                             {{-- Número de Fila --}}
                            <div class="flex-shrink-0 h-11 w-11 flex items-center justify-center bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-300 font-bold rounded-lg">
                                <span x-text="index + 1"></span>
                            </div>
                            
                            <div class="flex-grow">
                                <label class="mb-1.5 block text-xs font-semibold uppercase text-gray-500 tracking-wider">Método</label>
                                <div class="relative">
                                    ...
                                    <select 
                                        x-model="row.methodId" 
                                        {{-- MAGIA AQUÍ: Captura el TEXTO de la opción seleccionada --}}
                                    ...
                                    <i class="ri-arrow-down-s-line absolute top-1/2 right-3 -translate-y-1/2 text-gray-400 pointer-events-none"></i>
                        ...
                        {{-- 2. MONTO (Ancho MD: 3) --}}
                        <div class="md:col-span-3 md:pr-8 relative md:before:absolute md:before:right-0 md:before:top-1/2 md:before:-translate-y-1/2 md:before:h-8 md:before:w-px md:before:bg-gray-200 dark:md:before:bg-gray-700">
                            <label class="mb-1.5 block text-xs font-semibold uppercase text-gray-500 tracking-wider">Monto</label>
                            <div class="relative">
                                <span class="absolute top-1/2 left-3 -translate-y-1/2 text-gray-500 font-medium">S/.</span>
                                <input 
                                ...
                                <div class="space-y-3 animate-fade-in text-sm">
                                    <div class="grid grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <select :name="`payments[${index}][card_id]`" required class="w-full rounded-md border-gray-200 py-2 px-3 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:bg-dark-900">
                                                <option value="">Seleccionar</option>
                                                @foreach($cards as $card)
                                                    <option value="{{ $card->id }}">{{ $card->description }} ({{ $card->type }})</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div>
                                             <label class="block text-xs text-gray-500 mb-1">Procesador (POS)</label>
                                            <select :name="`payments[${index}][payment_gateway_id]`" class="w-full rounded-md border-gray-200 py-2 px-3 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:bg-dark-900">
                                    ...
                                    <div>
                                         <input type="text" :name="`payments[${index}][number]`" placeholder="N° Lote / Operación (Opcional)" class="w-full rounded-md border-gray-200 py-2 px-3 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:bg-dark-900">
                                    </div>
                                </div>
                            </template>
                                ...
                                <div class="space-y-3 animate-fade-in text-sm">
                                     <div class="grid grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <select :name="`payments[${index}][digital_wallet_id]`" required class="w-full rounded-md border-gray-200 py-2 px-3 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:bg-dark-900">
                                                <option value="">Seleccionar</option>
                                        ...
                                        <div>
                                            ...
                                            <input type="text" :name="`payments[${index}][number]`" required placeholder="Ej: 999..." class="w-full rounded-md border-gray-200 py-2 px-3 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:bg-dark-900">
                                        </div>
                                    </div>
                                </div>
                            </template>
                            ...
                            <template x-if="row.methodId == '3'">
                                <div class="space-y-3 animate-fade-in text-sm">
                                    <div>
                                        ...
                                        <select :name="`payments[${index}][bank_id]`" required class="w-full rounded-md border-gray-200 py-2 px-3 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:bg-dark-900">
                                            <option value="">Seleccionar</option>
                                    ...
                                    <div>
                                         <input type="text" :name="`payments[${index}][number]`" required placeholder="N° de Operación / Constancia" class="w-full rounded-md border-gray-200 py-2 px-3 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:bg-dark-900">
                                    </div>
                                </div>
                            </template>
        ...
        <div class="mt-5">
            <button type="button" @click="addNewRow()" class="inline-flex items-center gap-2 text-sm font-medium text-brand-600 hover:text-brand-700 transition-colors py-2 px-3 rounded-lg hover:bg-brand-50 dark:hover:bg-brand-900/20">
