...
@section('content')
    <div class="px-4 md:px-6 pt-4 pb-2">
        ...
        <div class="mx-4 md:mx-6 flex flex-col dark:bg-gray-950 overflow-hidden rounded-t-2xl">
            {{-- Misma estructura visual que orders/create (sin mesa, mozo, personas, delivery/llevar) --}}
            <div data-turbo-cache="false"
                class="flex flex-col flex-1 min-h-0 h-[calc(100vh-theme(spacing.52))] lg:h-[calc(100vh-theme(spacing.40))]">
...
es                        style="max-height: 80vh;">
                        ...
                        <div id="aside-resumen" class="flex flex-col flex-1 min-h-0 overflow-hidden">
                            <div id="cart-container"
                                class="flex-1 overflow-y-auto min-h-[300px] lg:min-h-0 overflow-x-hidden p-3 sm:p-5 space-y-2 sm:space-y-3 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700"
    ...
    <script>
        (function () {
                ...
                productsToShow.forEach((prod) => {
                    ...
                    el.innerHTML =
                        `
                                ...
                                ${hasImg
                            ? `<img src="${imageUrl}" alt="${safeName}" class="w-full h-full object-contain rounded-full object-cover object-center" loading="lazy" onerror="this.onerror=null; this.parentElement.innerHTML='<i class=&quot;ri-restaurant-2-line text-2xl sm:text-3xl text-white&quot;></i>';">`
            ...
            async function sendThermalTicketAfterSale(movementId, saleResponse) {
                ...
                if (qzApi && await ensureQzTrayConnected(qzApi, preferredPrinterName)) {
                    try {
                        ...
                        if (!tr.ok || !td?.success || (!td?.ticket_pdf_b64 && !td?.payload_b64)) {
                            await reportThermalPrintFailure(movementId, td?.print_job_id || null, td?.message || 'No se pudo obtener el ticket del servidor.', preferredPrinterName);
                        ...
                        await confirmThermalPrintJob(td.print_job_id, movementId, printerName);
                        showCobroNotification('Impresión', 'Ticket enviado a "' + printerName + '".', 'success');
                    } catch (e) {
                        markQzTrayUnavailable();
                        console.warn('QZ Ticket:', e);
                        await reportThermalPrintFailure(movementId, null, e?.message || 'No se pudo imprimir con QZ Tray.', preferredPrinterName);
                        if (strictLocalQz) {
                            openSaleTicketPdfTab(movementId);
                            return;
                        }
                ...
                if (strictLocalQz) {
                    await reportThermalPrintFailure(movementId, null, 'QZ Tray no esta disponible para esta ticketera.', preferredPrinterName);
                    openSaleTicketPdfTab(movementId);
                    return;
                }
                ...
                try {
                    ...
                    if (tr.ok && td?.success) {
                        ...
                        await reportThermalPrintFailure(movementId, td?.print_job_id || null, td?.message || 'No se pudo enviar a la ticketera.', preferredPrinterName);
                        openSaleTicketPdfTab(movementId);
                    }
                } catch (e) {
                    console.warn('Ticketera red:', e);
                    await reportThermalPrintFailure(movementId, null, e?.message || 'Error de red al imprimir.', preferredPrinterName);
                    openSaleTicketPdfTab(movementId);
                }
            }
