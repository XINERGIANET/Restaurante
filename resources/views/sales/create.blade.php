...
@section('content')
    ...
    <script>
        (function() {
            ...
            function printSaleTicketAfterSale(movementId) {
                if (!movementId) return;
                try {
                    const ticketUrl = getSaleTicketUrl(movementId);
                    const printWindow = window.open(ticketUrl, '_blank');
                    if (!printWindow) {
                        showCobroNotification('Impresión', 'El navegador bloqueó la ventana del ticket. Habilita pop-ups para este sitio.', 'error');
                        return;
                    }
                    showCobroNotification('Impresión', 'Se abrió el ticket de venta para imprimir.', 'success');
                } catch (e) {
                    console.warn('Abrir ticket:', e);
                    showCobroNotification('Impresión', 'No se pudo abrir el ticket de venta.', 'error');
                }
            }
            ...
            async function processSale() {
                ...
                try {
                    ...
                    printSaleTicketAfterSale(movementId);
                    setTimeout(() => {
