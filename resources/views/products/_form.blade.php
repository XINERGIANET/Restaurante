        ...
        <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                    ...
                    @isset($product->id)
                        <span class="ml-1 text-xs text-gray-400 dark:text-gray-500">(Gestionar en movimientos de
                            almacén)</span>
                    @endisset
                </label>
                <input type="number" step="0.01" value="{{ old('stock', $productBranch->stock ?? '') }}"
                    @isset($product->id) disabled @else name="stock" required @endisset
                    min="{{ old('stock_minimum', $productBranch->stock_minimum ?? 0) }}"
                    max="{{ old('stock_maximum', $productBranch->stock_maximum ?? 0) }}"
                    class="dark:bg-dark-900 shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm placeholder:text-gray-400 dark:border-gray-700 dark:text-white/90 dark:placeholder:text-white/30 {{ isset($product->id) ? 'bg-gray-100 dark:bg-gray-800 cursor-not-allowed text-gray-500' : 'bg-transparent focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 text-gray-800 focus:ring-3 focus:outline-hidden dark:bg-gray-900' }}"
                    placeholder="0.00" />
                @isset($product->id)
                    <input type="hidden" name="stock" value="{{ $productBranch->stock ?? 0 }}" />
                @endisset
            </div>
            ...
            <div>
                ...
                <input type="number" name="minimum_purchase" step="0.01"
                    value="{{ old('minimum_purchase', $productBranch->minimum_purchase ?? '') }}"
                    x-bind:disabled="platosCategoriaIds.includes(Number(categoryId))"
                    x-effect="if (platosCategoriaIds.includes(Number(categoryId))) $el.value = null"
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Unidad de
                    venta</label>
                <input type="text" name="unit_sale"
