        ...
        <form method="GET" class="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <x-ui.button size="sm" variant="outline" class="rounded-xl"
                    @click="window.location.href='{{ route('admin.banks.index') }}'">Limpiar</x-ui.button>
            </div>
        </form>
        <x-ui.button size="md" variant="create" @click="$dispatch('open-create-bank-modal')"><i
                class="ri-add-line"></i> Crear Banco</x-ui.button>
    </div>
    @if ($banks->count() > 0)
        <div
...
<x-ui.modal x-data="{ open: false }" @open-create-bank-modal.window="open = true"
    ...
    <div class="p-6 space-y-4">
        ...
        @endif
        <form id="create-bank-form" class="space-y-4" action="{{ route('admin.banks.store') }}" method="POST">
            @csrf
            @include('banks._form')
...
<x-ui.modal x-data="{ 
    ...
    <div class="p-6 space-y-4">
        ...
        <form class="space-y-4" :action="`{{ route('admin.banks.index') }}/${bankData.id}`" method="POST">
            @csrf
            @method('PUT')
