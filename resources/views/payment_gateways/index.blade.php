@extends('layouts.app')
@section('content')
<x-common.page-breadcrumb pageTitle="{{ 'Medios de pago' }}" />
<x-common.component-card title="Listado de medios de pago" desc="Gestiona los medios de pago registrados en el sistema.">
        ...
        <form method="GET" class="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <x-ui.button size="sm" variant="outline" class="rounded-xl"
                    @click="window.location.href='{{ route('admin.payment_gateways.index') }}'">Limpiar</x-ui.button>
            </div>
        </form>
        <x-ui.button size="md" variant="create" @click="$dispatch('open-create-payment-gateway-modal')"><i
                class="ri-add-line"></i> Crear Medio de Pago</x-ui.button>
    </div>
                            ...
                            <tr
                                ...
                                <td class="px-5 py-4 sm:px-6 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('admin.payment_gateways.destroy', $paymentGateway) }}"
                                            method="POST"
                                            class="relative group js-swal-delete"
                                            data-swal-title="Eliminar medio de pago?"
...
<!--Modal de creacion de medio de pago-->
<x-ui.modal x-data="{ open: false }" @open-create-payment-gateway-modal.window="open = true"
    ...
    <div class="p-6 space-y-4">
        <h3 class="mb-6 text-lg font-semibold text-gray-800 dark:text-white/90">Crear Medio de Pago</h3>
        @if ($errors->any())
            <div class="mb-5">
        ...
        @endif
        <form id="create-payment-gateway-form" class="space-y-4" action="{{ route('admin.payment_gateways.store') }}"
            method="POST">
            @csrf
