        ...
        <div class="flex flex-col gap-4">
            <form method="GET" action="{{ route('kardex.index') }}" class="mb-6">
                ...
                <div class="flex flex-col xl:flex-row gap-2 items-end">
                    ...
                    <div class="w-full">
                        ...
                        <select name="movement_type"
                            ...
                            @foreach ($availableTypes as $type)
                                <option value="{{ $type }}" @selected($typeFilter == $type)>{{ \Illuminate\Support\Str::limit($type, 12) }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    {{-- 4. From Date --}}
                    <div class="w-full">
                        <x-form.date-picker name="date_from" label="Desde" placeholder="Inicio" :defaultDate="$dateFrom" dateFormat="Y-m-d" class="h-10 !mb-0" />
                    </div>

                    {{-- 5. To Date --}}
                    <div class="w-full">
                        <x-form.date-picker name="date_to" label="Hasta" placeholder="Fin" :defaultDate="$dateTo" dateFormat="Y-m-d" class="h-10 !mb-0" />
                    </div>

                    {{-- 6. Buttons --}}
                    <div class="flex gap-1 w-full">
                        <x-ui.button type="submit" size="sm" variant="primary" style="background-color: #63B7EC; border-color: #63B7EC;">
                            <i class="ri-search-line"></i>
                            Buscar
                        </x-ui.button>
                        <x-ui.link-button href="{{ route('kardex.index', $viewId ? ['view_id' => $viewId] : []) }}" size="sm" variant="outline">
