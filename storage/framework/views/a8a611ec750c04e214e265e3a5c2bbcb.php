<?php $__env->startSection('content'); ?>
    ...
    <script>
        ...
        function renderProducts() {
            ...
            productsDB.forEach(prod => {
                console.log('Rendering product:', prod.id, prod.name);
                const productBranch = productsBranches.find(p => p.id === prod.id);
                console.log('Found productBranch:', productBranch);
                
                // Si no hay productBranch (producto no está en esta sucursal), no mostrar
                if (!productBranch) {
                    console.log('⚠️ Product not available in this branch:', prod.id, prod.name);
                    return;
                }
                
                console.log('✅ Product available - Price:', productBranch.price);
                
                const el = document.createElement('div');
                el.className = "group cursor-pointer";
                el.onclick = () => addToCart(prod, productBranch);

                el.innerHTML = `
            <div class="rounded-lg overflow-hidden p-2 shadow-sm hover:shadow-lg border border-gray-200 dark:border-slate-700/50 transition-all duration-200 hover:-translate-y-1">
                <!-- Imagen del producto -->
                <div class="relative aspect-square overflow-hidden bg-gray-100 dark:bg-slate-700/30 rounded-lg">
                    <img src="${prod.img}" 
                        alt="${prod.name}" 
                ...
                <div class="mt-3 flex flex-col gap-1">
                    <div class="flex justify-between items-center">
                        ...
                        <h4 class="text-xs text-gray-500 dark:text-gray-400">$${productBranch.price}</p>
                    </div>
        ...
        function addToCart(prod, productBranch) {
            ...
            if (existing) {
                ...
                currentSale.items.push({
                    pId: prod.id,
                    qty: 1,
                    price: productBranch.price, // Usar precio de la sucursal
                    note: ""
                });
            }
            saveDB();
    ...
    // Usar precio de productBranch
    const itemPrice = productBranch.price;
             ...
             <div class="flex-1 min-w-0 flex flex-col justify-between">
                 <div class="flex justify-between items-start mb-2">
                     <div class="flex-1 min-w-0 pr-2">
                         ...
                         <p class="text-xs text-gray-500 dark:text-gray-400">$${itemPrice.toFixed(2)} c/u</p>
                     </div>
                     <div class="shrink-0">
                         <span class="font-bold text-blue-600 dark:text-blue-400 text-base">$${itemTotal.toFixed(2)}</span>
                     </div>
            </div>
                 <div class="flex justify-between items-center">
